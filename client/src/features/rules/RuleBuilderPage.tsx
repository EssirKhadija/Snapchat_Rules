  ...
  return (
    <div className="space-y-6">
      <header className="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
        <h1 className="text-2xl font-semibold">Créateur de règles</h1>
        <p className="mt-2 text-slate-600">Composez des conditions et des actions pour automatiser vos campagnes.</p>
      </header>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <label className="text-sm text-slate-600">Activée</label>
            </div>
            ...
            <div>
              ...
              <div className="mt-3 space-y-2">
                ...
                <button type="button" onClick={()=> appendCond({ id: 'c-'+Date.now(), field: 'spend', operator: 'gt', value: '0' })} className="rounded-md bg-slate-900 text-white px-4 py-2">Ajouter condition</button>
              </div>
            </div>
            ...
            <div className="mt-4">
              ...
              <div className="mt-3 space-y-2">
                ...
                <button type="button" onClick={()=> appendAct({ id: 'a-'+Date.now(), type: 'pause_campaign' })} className="rounded-md bg-slate-900 text-white px-4 py-2">Ajouter action</button>
              </div>
            </div>
        ...
        <div className="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
          ...
          <p className="mt-2 text-sm text-slate-600">Un résumé de la règle avant l'enregistrement.</p>
          <div className="mt-4">
            <pre className="rounded-md bg-slate-50 p-4 text-sm whitespace-pre-wrap">{previewText}</pre>
          </div>

          <div className="mt-6 flex gap-2">
            <button type="submit" disabled={saving} className="rounded-xl bg-emerald-600 px-4 py-2 text-white">Enregistrer la règle</button>
            <button type="button" onClick={()=> { /* reset form - omitted for brevity */ }} className="rounded-xl border px-4 py-2">Réinitialiser</button>
          </div>
        </div>
      </form>
    </div>
